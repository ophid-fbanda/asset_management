...
CREATE VIEW public.events_view AS
 SELECT events.entity_id,
   ...
   FROM ( SELECT asset_registrations.id AS entity_id,
        ...
        UNION ALL
         SELECT asset_transfers.id,
           ...
           FROM asset_transfers
             JOIN stations receiving_station ON receiving_station.id = asset_transfers.receiving_station_id
        UNION ALL
         SELECT asset_issuances.id,
           ...
           FROM asset_issuances
             JOIN staff_profiles receiving_staff ON receiving_staff.id = asset_issuances.receiving_staff_id
        UNION ALL
        ...
        UNION ALL
         SELECT asset_disposals.id,
           ...
           FROM asset_disposals
             ...
             JOIN asset_brands ON asset_brands.id = asset_models.asset_brand_id
             JOIN asset_types ON asset_types.id = asset_brands.asset_type_id
             JOIN disposal_types ON disposal_types.id = asset_disposals.disposal_type_id) events
     ...
     LEFT JOIN staff_profiles staff_profiles2 ON staff_profiles2.id = event_approvals.event_admin_id;
