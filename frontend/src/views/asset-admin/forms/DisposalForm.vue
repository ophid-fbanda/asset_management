...
const disposalTypes  = dataFromCache('meta/disposal_types')
...
const submitForm = async () => {
  ...
  const response = await dataSend('assets/disposal', {
    ...form,
  ...
  if (response.status === 200) {
    objectReset(form)
    objectResetSet(ui, 'success', 'Disposal submitted successfully.')
  } else {
...
onMounted(() => {
  dataFetchToCache('meta/disposal_types')
})
</script>

<template>
  <div class="flex min-h-0 flex-1 flex-col gap-3">
    <h2 class="text-base font-semibold text-[#384884]">Dispose Asset</h2>
    ...
    <section class="flex w-full sm:w-1/2 flex-col gap-4 self-start rounded-md border border-[#c5cce3] bg-white p-4">
      ...
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-1.5">
          <label class="text-sm font-medium text-[#384884]">Disposal Type</label>
          <Select
            v-model="form.disposalTypeId"
        ...
        <div class="flex flex-col gap-1.5">
          <label class="text-sm font-medium text-[#384884]">Disposal Date</label>
          <DatePicker
