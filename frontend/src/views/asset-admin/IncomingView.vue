...
const dataKey = computed(() => (station.value != null ? `assets/incoming/${station.value}` : null))
...
const context = reactive({
  ...
  options: [
    {
      id: 'transferTemplate',
      name: 'Confirm incoming transfer',
      table: 1,
      target: 50,
      choices: [10, 11],
    },
...
const closeRouter = () => {
  ...
  if (station.value != null) {
    dataRefreshCache(`assets/station/${station.value}`)
    dataRefreshCache(`assets/changes/${station.value}`)
  }
}

const exportExcel = () => exportToExcel(filteredRecords.value, 'incoming-transfers', 'Incoming')
