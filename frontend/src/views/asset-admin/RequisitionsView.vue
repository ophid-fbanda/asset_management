...
// Live requisitions list for that station, cached per-station (load-once).
const dataKey = computed(() => (station.value != null ? `assets/requests/${station.value}` : null))
...
const context = reactive({
  ...
  collector: [],
  options: [
    { id: 'reqTemplate', name: 'Requisition', table: 1 }
  ]
})
...
// Open the router dialog with the collected rows riding along the options.
const collect = () => objectSet(context, 'dialog', true)

// New Requisition bypasses the selection options entirely: it names the form
// to mount via `external` and launches the dialog.
const newRequisition = () => {
  objectSet(context, 'external', 'requisitionForm')
...
const exportExcel = () => {
  exportToExcel(filteredRecords.value, 'requisitions', 'Requisitions')
}
