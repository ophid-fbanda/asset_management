...
const savePdf = () => exportToPDF(docEl.value, `PLC-EN${entity.value?.entity_id}EV${entity.value?.event_id}`, 'landscape', 'a4')

const dataKey   = computed(() => entity.value?.entity_id != null ? `assets/placement/${entity.value.entity_id}` : null)
const placement = computed(() => dataKey.value ? (dataFromCache(dataKey.value).value ?? {}) : {})
const details   = computed(() => placement.value.details   ?? {})
const approvals = computed(() => placement.value.approvals ?? [])
  ...
  <div ref="docEl" class="w-[297mm] min-h-[210mm] bg-white text-[#0f172a] text-[11px] overflow-hidden border border-[#e2e8f0]">
    ...
    <div class="px-[18mm] pt-[10mm] pb-4 border-b border-[#384884] flex items-center justify-between gap-6">
      ...
      <div class="flex flex-col items-center text-center">
        <span class="text-[13px] font-black uppercase tracking-tight text-[#384884]">Asset Placement Record</span>
    ...
    <div class="px-[18mm] py-[10mm]">
      ...
      <div class="grid grid-cols-3 gap-x-8 gap-y-3 mb-8">
        ...
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Placement Type</span>
          <span class="font-semibold">{{ details.placement_type }}</span>
        </div>
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Placement Date</span>
          <span class="font-semibold">{{ details.placement_date }}</span>
        </div>
      ...
      <div class="border-t border-[#e2e8f0] pt-6 flex flex-col">
        ...
        <div class="flex gap-10 pt-2 pb-1">
          <div class="flex-1 border-b border-[#94a3b8] flex items-end justify-between">
            ...
            <span class="text-[8px] text-[#64748b] pb-0.5">{{ details.placement_date }}</span>
          </div>
