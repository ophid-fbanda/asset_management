...
const savePdf = () => exportToPDF(docEl.value, `REQ-EN${entity.value?.entity_id}EV${entity.value?.event_id}`, 'landscape', 'a4')

const dataKey     = computed(() => entity.value?.entity_id != null ? `assets/request/${entity.value.entity_id}` : null)
const requisition = computed(() => dataKey.value ? (dataFromCache(dataKey.value).value ?? {}) : {})
const details     = computed(() => requisition.value.details   ?? {})
const assets      = computed(() => requisition.value.list      ?? [])
const approvals   = computed(() => requisition.value.approvals ?? [])
  ...
  <div ref="docEl" class="w-[297mm] min-h-[210mm] bg-white text-[#0f172a] text-[11px] overflow-hidden border border-[#e2e8f0]">
    ...
    <div class="px-[18mm] pt-[10mm] pb-4 border-b border-[#384884] flex items-center justify-between gap-6">
      ...
      <div class="flex flex-col items-center text-center">
        <span class="text-[13px] font-black uppercase tracking-tight text-[#384884]">Asset Requisition Record</span>
