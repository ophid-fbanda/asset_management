...
const savePdf = () => exportToPDF(docEl.value, `VRF-EN${entity.value?.entity_id}EV${entity.value?.event_id}`, 'landscape', 'a4')

const dataKey      = computed(() => entity.value?.entity_id != null ? `assets/verification/${entity.value.entity_id}` : null)
const verification = computed(() => dataKey.value ? (dataFromCache(dataKey.value).value ?? {}) : {})
const details      = computed(() => verification.value.details   ?? {})
const approvals    = computed(() => verification.value.approvals ?? [])
  ...
  <div ref="docEl" class="w-[297mm] min-h-[210mm] bg-white text-[#0f172a] text-[11px] overflow-hidden border border-[#e2e8f0]">
    ...
    <div class="px-[18mm] pt-[10mm] pb-4 border-b border-[#384884] flex items-center justify-between gap-6">
      ...
      <div class="flex flex-col items-center text-center">
        <span class="text-[13px] font-black uppercase tracking-tight text-[#384884]">Asset Verification Record</span>
    ...
    <div class="px-[18mm] py-[10mm]">
      ...
      <div class="grid grid-cols-3 gap-x-8 gap-y-3 mb-8">
        ...
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Verification Type</span>
          <span class="font-semibold">{{ details.verification_type }}</span>
        </div>
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Verified Condition</span>
          <span class="font-semibold">{{ details.verified_condition }}</span>
        </div>
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Verification Date</span>
          <span class="font-semibold">{{ details.verification_date }}</span>
        </div>
      ...
      <div class="border-t border-[#e2e8f0] pt-6 flex flex-col">
        ...
        <!-- Row 2: name + timestamp on the signature line -->
        <div class="flex gap-10 pt-2 pb-1">
          <div class="flex-1 border-b border-[#94a3b8] flex items-end justify-between">
            ...
            <span class="text-[8px] text-[#64748b] pb-0.5">{{ details.verification_date }}</span>
          </div>
