...
const savePdf = () => exportToPDF(docEl.value, `ISS-EN${entity.value?.entity_id}EV${entity.value?.event_id}`, 'landscape', 'a4')

const dataKey   = computed(() => entity.value?.entity_id != null ? `assets/issuance/${entity.value.entity_id}` : null)
const issuance  = computed(() => dataKey.value ? (dataFromCache(dataKey.value).value ?? {}) : {})
const details   = computed(() => issuance.value.details   ?? {})
const assets    = computed(() => issuance.value.list      ?? [])
const approvals = computed(() => issuance.value.approvals ?? [])
  ...
  <div ref="docEl" class="w-[297mm] min-h-[210mm] bg-white text-[#0f172a] text-[11px] overflow-hidden border border-[#e2e8f0]">
    ...
    <div class="px-[18mm] pt-[10mm] pb-4 border-b border-[#384884] flex items-center justify-between gap-6">
      ...
      <div class="flex flex-col items-center text-center">
        <span class="text-[13px] font-black uppercase tracking-tight text-[#384884]">Asset Issuance Record</span>
    ...
    <div class="px-[18mm] py-[10mm]">
      ...
      <div class="grid grid-cols-3 gap-x-8 gap-y-3 mb-8">
        ...
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Recipient</span>
          <span class="font-semibold">{{ details.recipient }}</span>
        </div>
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Issuance Type</span>
          <span class="font-semibold">{{ details.issuance_type }}</span>
        </div>
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Issuance Date</span>
          <span class="font-semibold">{{ details.issuance_date }}</span>
        </div>
      ...
      <div class="border-t border-[#e2e8f0] pt-6 flex flex-col">
        ...
        <div class="flex gap-6 pb-1">
          ...
          <div class="flex-1">
            ...
            >{{ recipientApproval ? recipientApproval.approval : 'Accepted by — Name, Signature &amp; Date' }}</span>
          </div>
        </div>
        ...
        <div class="flex gap-6 pt-2 pb-1">
          <div class="flex-1 border-b border-[#94a3b8] flex items-end justify-between">
            ...
            <span class="text-[8px] text-[#64748b] pb-0.5">{{ details.issuance_date }}</span>
          </div>
