...
const savePdf = () => exportToPDF(docEl.value, `INC-EN${entity.value?.entity_id}EV${entity.value?.event_id}`, 'landscape', 'a4')

const dataKey  = computed(() => entity.value?.entity_id != null ? `assets/incident/${entity.value.entity_id}` : null)
const incident = computed(() => dataKey.value ? (dataFromCache(dataKey.value).value ?? {}) : {})
const details  = computed(() => incident.value.details   ?? {})
const approvals = computed(() => incident.value.approvals ?? [])
...
const latestApprovalColor = computed(() => {
  const id = entity.value?.latest_approval_type_id ?? entity.value?.status_id ?? 0
  if (id === 0) return '#f59e0b'
  return id % 2 === 0 ? '#22c55e' : '#ef4444'
})

const latestApprovalLabel = computed(() => entity.value?.latest_approval ?? entity.value?.status ?? null)
...
<template>
  ...
  <div ref="docEl" class="w-[297mm] min-h-[210mm] bg-white text-[#0f172a] text-[11px] overflow-hidden border border-[#e2e8f0]">
    ...
    <div class="px-[18mm] pt-[10mm] pb-4 border-b border-[#384884] flex items-center justify-between gap-6">
      ...
      <div class="flex flex-col items-center text-center">
        <span class="text-[13px] font-black uppercase tracking-tight text-[#384884]">Asset Incident Record</span>
    ...
    <div class="px-[18mm] py-[10mm]">
      ...
      <div class="grid grid-cols-3 gap-x-8 gap-y-3 mb-8">
        ...
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Incident Type</span>
          <span class="font-semibold">{{ details.incident_type }}</span>
        </div>
        <div class="flex flex-col gap-0.5 border-b border-[#e2e8f0] pb-1.5">
          <span class="text-[8px] font-bold uppercase tracking-widest text-[#384884]">Incident Date</span>
          <span class="font-semibold">{{ details.incident_date }}</span>
        </div>
      ...
      <div class="border-t border-[#e2e8f0] pt-6 flex flex-col">
        ...
        <div class="flex gap-10 pt-2 pb-1">
          <div class="flex-1 border-b border-[#94a3b8] flex items-end justify-between">
            ...
            <span class="text-[8px] text-[#64748b] pb-0.5">{{ details.incident_date }}</span>
          </div>
  ...
  <!-- Appendices -->
  <div v-if="details.asset_image" class="w-[297mm] bg-white overflow-hidden border border-[#e2e8f0]">
    <div class="px-4 py-2 border-b border-[#e2e8f0] flex items-center gap-2">
      <i class="pi pi-paperclip text-[#384884] text-sm" />
      <span class="text-[10px] font-bold uppercase tracking-widest text-[#384884]">Asset Image:</span>
      <span class="text-[10px] text-[#64748b]">{{ details.asset_image }}</span>
    </div>
    <iframe
      :src="dataApiUrl(`files/${details.asset_image}`)"
      class="w-full h-[400px] border-0"
      title="Asset Image"
    />
  </div>

  <div v-if="details.police_report" class="w-[297mm] bg-white overflow-hidden border border-[#e2e8f0]">
    <div class="px-4 py-2 border-b border-[#e2e8f0] flex items-center gap-2">
      <i class="pi pi-paperclip text-[#384884] text-sm" />
      <span class="text-[10px] font-bold uppercase tracking-widest text-[#384884]">Police Report:</span>
      <span class="text-[10px] text-[#64748b]">{{ details.police_report }}</span>
    </div>
    <iframe
      :src="dataApiUrl(`files/${details.police_report}`)"
      class="w-full h-[400px] border-0"
      title="Police Report"
    />
  </div>
